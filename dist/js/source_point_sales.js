 ...
 function agrega_a_lista(){
   $(document).ready(function(){
         if($("#cantidad").val()>0){
            ...
            var periodogarantia=$("#periodogarantia").val();
            var descripcion=$(".widget-user-desc").html();
            var precio=$("#preciou").val();
            var cantidad=$("#cantidad").val();
            var monto=cantidad*precio;
            $("#tabla_articulos > tbody").append("<tr><td class='center'>"+articulo+"</td><td class='center'>"+periodogarantia+"</td><td class='center'>"+descripcion+"</td><td class='center'>"+cantidad+"</td><td class='center'>"+precio+"</td><td class='center'>"+monto.toFixed(2)+"</td><td class='center'><button class='btn btn-block btn-danger btn-xs delete'><i class='icon-trash bigger-120'></i> Eliminar</button></td></tr>");
            $("#codigo").val("");
            $("#periodogarantia").val("");
            $("#cantidad").val("");
            $("#preciou").val("");
...
/**************************************************************************************/
function procesa_reclamacion(){
  $(document).ready(function(){
    /*busca el numero de ticket*/
     var n_tic='';
      $.ajax({
          url: 'busca_ticket_recla.php',
          type: 'POST',
          data: null,
          success: function(x){
            n_tic=x;
      ...
      ////******************************/////
     setTimeout('actualiza_ticket_recla()',1500);
          ...
          //$("#btn-procesa").prop('disabled', true);
           ...
           var yapuso=0;
                         ...
                         $.ajax({
                             ...
                             url: 'procesa_reclamacion.php',
                             type: 'POST',
                             data: 'descripcion='+$("#descripcion").val()+'&estado='+$("#estado").val()+'&detalles= &idfactura='+$("#idfactura").val()+'&idgarantia='+$("#idgarantia").val(),
                             success: function(x){
                                  var n = noty({
                                   text: "Procesando reclamo ",
                                   theme: 'relax',
                                   layout: 'topLeft',
                                   type: 'success',
                                   timeout: 2000,
                               ...
                               if(yapuso==0){
                               llena_ticket_recla($("#descripcion").val(),$("#estado").val(),$("#idfactura").val(),$("#idgarantia").val(),n_tic);
                               yapuso=1;
                               }else{
                               llena_ticket_recla($("#descripcion").val(),$("#estado").val(),$("#idfactura").val(),$("#idgarantia").val(),n_tic);
                               }
                        ...
                        /**************************************************************************************/
                        function actualiza_reclamacion(id){


                                                 $.ajax({
                                                     beforeSend: function(){
                                                      },
                                                     url: 'actualiza_reclamacion.php',
                                                     type: 'POST',
                                                     data: 'detalles='+$("#detalles").val()+'&estado='+$("#estado").val()+' &idfactura='+$("#idfactura").val()+'&idgarantia='+$("#idgarantia").val()+'&id='+id,
                                                     success: function(x){
                                                          var n = noty({
                                                           text: "Procesando reclamo ",
                                                           theme: 'relax',
                                 ...
                                 var clients='0';
                                  //$("#btn-procesa").prop('disabled', true);
                                   ...
                                   $('#tabla_articulos > tbody > tr').each(function(){
                                        ...
                                        var periodogarantia1 = $(this).find('td').eq(1).html();
                                        ...
                                        var monto=$(this).find('td').eq(5).html();
                                                 $.ajax({
                                                     ...
                                                     type: 'POST',
                                                     data: 'codigo='+cod+'&cantidad='+can+'&preciou='+preciou+'&credito='+credi+'&clienteid='+clients+'&caja='+$("#ncaja").val()+'&periodogarantia='+periodogarantia1,
                                                     success: function(x){
                                                          var n = noty({
...
/*******************************************************************************************/
function actualiza_ticket_recla(){
  $(document).ready(function(){
    $.ajax({
                             beforeSend: function(){
                              },
                             url: 'update_numero_ticket_recla.php',
                             type: 'POST',
                             data: null,
                             success: function(x){
                               //alert("Se actualizo el numero de ticket");
...
function llena_ticket_recla(param1,param2,param3,param4,param5){
   var descripcion=param1;
   var estado=param2;
   var idfactura=param3;
   var idgarantia=param4;
   var serie=2;
   var detalle=" ";
   var nn=param5;
   $.ajax({
        ...
        url: 'impresion_tickets_recla.php',
        type: 'POST',
        data: 'descripcion='+descripcion+'&estado='+estado+'&idfactura='+idfactura+'&idgarantia='+idgarantia+'&serie='+serie+'&detalle='+detalle+'&numero_ticket='+nn,
        success: function(x){
         //alert(x);
         },
        error: function(jqXHR,estado,error){
